"""
A2C (Advantage Actor-Critic) dla arbitrażu WIG20-DAX - PyTorch GPU Version

🚀 PYTORCH GPU VERSION - Znacznie szybsze uczenie!

ZMIANY vs TensorFlow wersja:
- TensorFlow/Keras → PyTorch
- Automatyczne wykorzystanie GPU (CUDA)
- Szybsze operacje tensorowe
- Lepsza kontrola nad uczeniem

KLUCZOWA RÓŻNICA vs Policy Gradient:
...
print(f"\n{'=' * 70}")
print(f"🚀 PyTorch A2C - GPU Version")
print(f"{'=' * 70}")
...
class ActorNetwork(nn.Module):
    ...
    def get_action_probs(self, state):
        """Zwraca prawdopodobieństwa akcji z temperature scaling"""
...
class A2CAgent:
    def __init__(self, state_size=5, action_size=3,
        ...
        """
        A2C Agent z PyTorch - automatycznie używa GPU jeśli dostępne
        """
        ...
        # Utwórz sieci i przenieś na GPU
        self.actor = ActorNetwork(state_size, action_size, temperature).to(device)
        ...
        print(f"\n🎭 A2C Agent utworzony (PyTorch + {device}):")
    ...
    def act(self, state, greedy=False):
        ...
        if greedy:
            return np.argmax(probs_cpu)

        # Epsilon-greedy
        if np.random.rand() < self.epsilon:
            return np.random.randint(self.action_size)

        # Sample z prawdopodobieństw
        return np.random.choice(self.action_size, p=probs_cpu)

    def train(self, states, actions, rewards, next_states, dones):
        """
        A2C Training - PyTorch version

        Args:
            states: lista states
            actions: lista actions
            rewards: lista rewards
            next_states: lista next_states
            dones: lista done flags
        """
        self.actor.train()
        self.critic.train()

        # Konwertuj na tensory i przenieś na GPU
        ...
        logits_scaled = logits / self.actor.temperature
        logits_scaled = torch.clamp(logits_scaled, -2.0, 2.0)

        # Prawdopodobieństwa akcji
        action_probs = F.softmax(logits_scaled, dim=-1)

        # Log prawdopodobieństwa dla wybranych akcji
        ...
        entropy_coef = 0.01
        actor_loss = -(action_log_probs * advantages).mean() - entropy_coef * entropy
        ...
        return {
            ...
            'mean_td_target': td_targets.mean().item(),
...
try:
    # UWAGA: Format używa średnika (;) i MA HEADER w pierwszej linii!
    ...
    # Kolumna nazywa się 'datetime' (z headera)
    ...
    if first_hour <= 3:  # Prawdopodobnie UTC
        hours_offset = 7  # UTC -> CET/CEST w czasie godzin giełdowych
        print(f"\n✅ Wykryto dane w UTC (pierwsza godzina: {first_hour}:00)")
        print(f"Dodaję {hours_offset}h aby dostać czas lokalny")
    elif first_hour >= 8:  # Prawdopodobnie już czas lokalny
        hours_offset = 0
        print(f"\n✅ Wykryto dane już w czasie lokalnym (pierwsza godzina: {first_hour}:00)")
    else:
        hours_offset = 6  # Domyślnie jak było wcześniej
    ...
    # Weryfikacja: sprawdź ile danych jest w godzinach giełdowych (9-17)
    ...
    if trading_hours_pct < 30:
        ...
        print("Sprawdź ręcznie dane i dostosuj hours_offset jeśli potrzeba")
    else:
    ...
    print("\nSprawdź:")
    print("1. Czy masz plik data/ger40_m1.csv")
    print("2. Czy pierwsza linia to header: datetime;open;high;low;close;volume")
    print("3. Czy separator to średnik (;)")
    print("4. Czy format datetime to: YYYYMMDD HHMMSS (np. 20220103 020000)")
    import traceback
...
# Features list - 18 NAJWAŻNIEJSZYCH (jak w wersji 400 days)
features = [
    # WIG20 (6 features)
    'wig20_returns',
...
print(f"\n{'=' * 70}")
print("🎭 Tworzę A2C Agenta (PyTorch)...")
print(f"{'=' * 70}\n")
...
# ============================================
#   TRENING A2C Z BATCH (PyTorch)
# ============================================

print(f"\n{'=' * 70}")
print("🚀 Rozpoczynam trening A2C (PyTorch + GPU)...")
print(f"{'=' * 70}\n")
...
print(f"   Epizod = {TRADING_DAY_MINUTES} minut")
...
for episode in range(episodes):
    ...
    if (episode + 1) % BATCH_SIZE == 0:
        # Pokaż prawdopodobieństwa
        with torch.no_grad():
            state_tensor = torch.FloatTensor(state).unsqueeze(0).to(device)
            probs = agent.actor.get_action_probs(state_tensor).cpu().numpy()[0]

            state_tensor_critic = torch.FloatTensor(state).unsqueeze(0).to(device)
            value = agent.critic(state_tensor_critic).cpu().numpy()[0][0]
    ...
    if (episode + 1) % BATCH_SIZE == 0:
        ...
        tqdm.write(f"   Trenuję na {len(batch_states)} krokach (GPU)...")
        ...
        tqdm.write(f"   Entropy: {train_stats['entropy']:.4f}")
        ...
        if val_reward > best_val_reward:
            ...
            agent.save('best_a2c_actor_pytorch.pth', 'best_a2c_critic_pytorch.pth')
...
plt.tight_layout()
plt.savefig('a2c_arbitrage_pytorch_results.png', dpi=150)
print("\n✓ Wykres zapisany: a2c_arbitrage_pytorch_results.png")
...
print(f"\n{'=' * 70}")
print("✅ TRENING A2C ZAKOŃCZONY (PyTorch GPU)!")
print(f"{'=' * 70}")
print(f"\nBest Val Reward: {best_val_reward:.2f}")
print(f"Test Reward: {test_reward:.2f}")
print(f"\nModele zapisane (PyTorch format):")
print(f"  - best_a2c_actor_pytorch.pth")
print(f"  - best_a2c_critic_pytorch.pth")
print(f"\n{'=' * 70}")